...
function openNewChatWindow(msg, mid, username){
	if(countChatWindows() >= chat.MAX_CHAT_WINDOW){
		alert("Sorry you cant open more windows\nSoon I will device a way  to allow you do so.\n\n");return;
	}
	//this.minimizeChatWindow();
	//this.markAsActiveChat(username);
			
	if(!isUserExist(username)){//If Window is not Opened already
	...
	}else if((mid != undefined) && (mid != null)){//If window is Oppened and msg is from outside
		alert("Append 2")
		appendMessage(username, msg, mid);
	}else{//If from self if implies that the window is already opened so restore
...
function searchUser(word){
	$.ajax({
		...
		data: "source=search&word="+word+"&me="+chat.me,
...
function loadUsersList(users){
	...
	for(var i=0; i<users.length; i++){
		var is_onlineClass=(users[i].is_online == 1)? 'class="chat-online"' : 'class="chat-offline"';
		
		html +='<li onlineStatus="'+users[i].is_online+'" activeChat="0" data-user-id="'+users[i].username+'"> <a href="javascript:void(0)"  title="Chat with: '+users[i].firstname+' '+users[i].lastname+'"><span class="chatuser">'+users[i].username+'</span> <span '+ is_onlineClass +'></span></a></li>';	
	}
...
function displayChatWindows(html){
...
function clickChat(){
	...
	$.ajax({
		...
		success:function(s){
			$('#chatList ul').html(s);},
		error:function(){
			$('#chatList ul').html('<li style="color:red"><span>&times;</span> Failed to load users for chat</li>');} ,
		beforeSend:function(){
			$('#chatList ul').html('<li><img src="/images/loading.gif"> Loading Users </li>');} ,
	});
	$('#chatList').css({'left':(x-20)+'px'}).slideToggle('slow');
	
	timeout = setTimeout(function(){$('#chatList').hide('slow');},100000);
};
...
//		iSendLast(username);
		...
		if(chatBody.find('p').length>0){//If there is message already 
			if(chatBody.find('p:last').attr('class') == "triangle-right"){//and I am the last sender
				...
				chatBody.find('p:last').append('<hr msgId="'+mid+'" />1. '+msg);
			}else{
				console.log("2. Appending to: "+chatBody.find('p:last').html());
				chatBody.find('p:last').after('<p class="triangle-right" msgId="'+mid+'">2. '+msg+'</p>');	
			}
		}else{//First time msg
			chatBody.html('<p class="triangle-right" msgId="'+mid+'">3. '+msg+'</p>');
		}
	}else{
console.log("appendMessage:2 "+$('#chatArea div[id="'+username+'"] #chatBody[user="'+username+'"]').html());
		...
		if(chatBody.find('p').length>0){//If there is message already 
			if(chatBody.find('p:last').attr('class') == "triangle-right"){//and I am NOT the last sender
				...
				chatBody.find('p:last').after('<p class="triangle-right top" msgId="'+mid+'">4. '+msg+'</p>');	
			}else{
				chatBody.find('p:last').append('<hr msgId="'+mid+'" />1. '+msg);
				console.log("5. Appending to: "+chatBody.find('p:last').html());
			}
		}else{//First time msg
			chatBody.html('<p class="triangle-right top" msgId="'+mid+'">6. '+msg+'</p>');
		}
	}
